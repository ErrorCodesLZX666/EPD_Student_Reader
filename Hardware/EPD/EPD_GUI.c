...
void EPD_ShowChinese(u16 x,u16 y,u8 *s,u8 sizey,u16 color)
{
	while(*s!=0)
	{
		...
		if (*s >= 0x20 && *s <= 0x7e) {
			...
			{
			case 12:
				EPD_ShowChar(x,y,*s,12,color);
				break;
			case 16:
				EPD_ShowChar(x,y,*s,16,color);
				break;
			case 24:
				EPD_ShowChar(x,y,*s,24,color);
				break;
...
void EPD_ShowChinese12x12(u16 x,u16 y,u8 *s,u8 sizey,u16 color)
{
	...
	u8 *dzk = mymalloc(SRAMIN,TypefaceNum);
	if (dzk == NULL) return;
	memset(dzk, 0, TypefaceNum);
	Get_HzMat(s,dzk,sizey);	//得到相应大小的点阵数据 
	u8 temp,t,t1;
	u16 y0=y;
	for(t=0;t<TypefaceNum;t++)
	{   												   
		temp=dzk[t];			//得到点阵数据                          
		for(t1=0;t1<8;t1++)
		{
            if(temp&0x80){ Paint_SetPixel(x,y,color); } //LCD_DrawPoint(x,y,fc);
			temp<<=1;
			y++;
			if((y-y0)==sizey)
			{
