...
/*
  修改说明（针对 Keil MDK v6 / ARMCLANG）：
  - 删除了 __attribute__((at(...))) 固定地址，将内存池放入默认的 RW/ZI 段（内部 SRAM）
  - 使用标准的 aligned 属性保证 32 字节对齐
  - 确保代码中仅通过 mallco_dev.membase[memx] / mallco_dev.memmap[memx] 访问内存
*/

/* 内存池(32字节对齐)，改为不固定地址，由链接器放在内部 SRAM */
__attribute__((aligned(32))) u8 mem1base[MEM1_MAX_SIZE];   /* 内部SRAM内存池 */
//__attribute__((aligned(32))) u8 mem2base[MEM2_MAX_SIZE];   /* 原计划为外部SDRAM，但当前板子无SDRAM，放到内部SRAM作为备用 */
__attribute__((aligned(32))) u8 mem3base[MEM3_MAX_SIZE];   /* 内部CCM内存池（若MCU有CCM，链接器可能把它放合适段） */
...
/* 内存管理控制器结构体（保留原有接口） */
/* 注意：这里直接初始化为数组变量（不含 at(...)） */
struct _m_mallco_dev mallco_dev =
{
    my_mem_init,
    my_mem_perused,
    mem1base, NULL, mem3base,        /* SDRAM 设置为 NULL */
    mem1mapbase, NULL, mem3mapbase,  /* SDRAM map 也设为 NULL */
    0, 0, 0
};
