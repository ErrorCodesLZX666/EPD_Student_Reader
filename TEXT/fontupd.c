...
// 定义方法代表字库是否更新成功！
uint8_t FONT_loading_success_flag = 0;      // 0默认是不成功
...
{
	LOGD("Trying to update FONT to flash ... \r\n");
	u32 flashaddr=0;								    
	FIL * fftemp;
	u8 *tempbuf;
 	u8 res;	
 	...
 	if(rval==0)	 
	{
        ...
        LOGD("Now trying to Copy,Progress ... \r\n");
...
{	
	...
	{  
		...
		LOGE("------------------ [FONT COPY ERROR] ------------------ \r\n");
		LOGE("Error Report : \r\n");
		// 有错误，输出错误信息
		// 尝试判断字库是否有误
		if (rval&1<<4) LOGE("FONT File Not Found = %s \r\n",UNIGBK_PATH);
		else if (rval&1<<5)LOGE("FONT File Not Found = %s \r\n",GBK12_PATH);
		else if (rval&1<<6)LOGE("FONT File Not Found = %s \r\n",GBK16_PATH);
		else if (rval&1<<7)LOGE("FONT File Not Found = %s \r\n",GBK24_PATH);
		LOGE("Error cased FONT copy program exit ... \r\n");
		LOGE("--------------------------------------------------------\r\n");
	}
...
{		
	...
	while(t<10)//连续读取10次,都是错误,说明确实是有问题,得更新字库了
	{
		...
		delay_1ms(50);
		LOGE("Loading Font failed !!! Retry times = %d\r\n",t);
	}
	if(ftinfo.fontok!=0XAA)
    {
		LOGE("Max font loading retry reached,FONT maybe not exist !!!\r\n");
        //font=0;
        return 1;
    }
    else
    {
		LOGD("FONT loading success inear !!!\r\n");
        //font=1;
        return 0;		  
    }
